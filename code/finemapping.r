...
option_list = list(
    make_option(c("-g", "--geno"), type="character", default=NULL, help="genotypes file", metavar="character"),
    make_option(c("-p", "--pheno"), type="character", default=NULL, help="phenotypes file", metavar="character"),
    make_option(c("-c", "--cov"), type="character", default=NULL, help="covariates file", metavar="character"),
...
### LOAD DATA
###################
dosage <- read.table(opt$geno, header=T, stringsAsFactors=F)
print(dim(dosage))

pheno <- scan(opt$pheno)

PCA <- read.table(opt$cov, header=F, stringsAsFactors=F, sep=",")
#PCA = prcomp(t(dosage), scale = TRUE)
...
if(length(hits1)==0){
  ...
  repos <- result[result$model_size==1, -c(1, 20, 24, 26, 27)]

}else{
  # set model
  constrain <- rep(0, 1 + length(hits1) + dim(PCA)[2])
  constrain[1 + which(traits == "CD")] <- 1
  ...
  while(TRUE){
    ...
    for (hiti in 1:length(hits1)){
      if(constrain[hiti+1]==3){
        ...
        rr <- lapply(names(subset(dosage,  select=r2>MIN_LD_REPOS) ), reposTest, dosage, as.matrix(x_other), xs, hiti, pheno, constrain)
  ...
  for (hiti in c(1:nhit)){
    ...
    ret <- do.call("rbind", rr)
    repos <- rbind(repos,  data.frame(ret[, c(2, 3)], model_size=cnt, ret[,c(5:7, 9:10, 12)]))
    cnt <- cnt +1
  }

}

write.table(repos, file=paste0(opt$out,"_repos.txt"), col.names=TRUE, row.names=F, quote=F, sep="\t")
...
write.table(output, file=paste0(opt$out,"_credible.txt"), col.names=TRUE, row.names=F, quote=F, sep="\t")
