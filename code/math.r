...
multinomConstrain <- function(y,xs,constrain){
 ...
 for (i in 1:Npred){
    ...
    nnet.default(X, Y, size = 0,skip = TRUE, softmax = TRUE, rang=0, trace=FALSE, na.action="na.omit")
}
...
getlogLik <- function(y, covar, model_spec, dat ){
	...
	# set model
	var_total <- length(model_spec$CD_SNP)

	constrain <- rep(0,var_total + 1 + ncovar)
	if(length(model_spec$CD_SNP) > 0){
		constrain[1 + 1:length(model_spec$CD_SNP)] <- 1
	}

	model_element <- dat[,  c(model_spec$CD_SNP) ]
	...
	if(P_cd==0){
		...
		param_CD <- param[(1+1+P+ncovar+1+1+1): (1+1+P+ncovar+1+1+1 + P-1) ]
	}
...
growModel <- function(model_spec, i, trait, dat){
	...
	existing_snp <- c(model_spec$CD_SNP)
	...
	if(i %in% existing_snp){
		...
		if(trait=="CD"){
			...
			new_pos <- length(model_spec$CD_SNP)
		}
		model_spec$good <- 1;
...
getSpec <- function(test_ret){
	...
	model_spec <- list(CD_SNP=NULL, good=1)
...
modelProb<-function(y, covar, model_spec, dat, Neff){
  ...
  if(sum(!is.na(ret)) > 0 ){
    BIC_penalty <- -log(ret$N_cd)/2*(ret$P_cd)
    model_penalty <- -lchoose(Neff, length(ret$CD_SNP))
...
testSNP <- function(i, y, covar, Neff, prev_best_model, null_logProb, dat){
  ...
  for (d in c("CD")){
    ...
    temp <- data.frame(CD_SNP=paste(current_model$CD_SNP, collapse="@"),
                       ...
                       add_trait=d,
                       model_size=length(current_model$CD_SNP),
                       n_param=length(current_model$CD_SNP),
...
modelSearch <- function(dosage, pheno, PCA, Neff){
  ...
  null_model <- modelProb(pheno, PCA, list(CD_SNP=NULL, good=1), dosage,  Neff)
  ...
  while(TRUE){
    ...
    if(iter==0){
      ...
      print(paste("Prev best model is:", prev_best_model$CD_SNP) )
      print(paste("Current best model is:", best_model$CD_SNP) )
    ...
    if( best_model$logProb - prev_best_model$logProb <= 0  ){
      ...
      print(paste("Current relative logProb is", sprintf("%.2f",  best_model$logProb - null_model$logProb) ))
      print(paste("Current best model is CD:", best_model$CD_SNP) )
      break;
    }
